...
export function Dashboard({ }: DashboardProps) { // Removed events from props
    const [events, setEvents] = useState<ProcessedEvent[]>([]); // Added events state
    ...
    useEffect(() => {
        const loadData = async () => {
            try {
                ...
                // Map SQL result to ProcessedEvent interface
                const mappedEvents: ProcessedEvent[] = reportData.map((row: any) => {
                    const totalAudience = Number(row.publico_total || 0);
                    const totalPayingQty = Number(row.pagantes || 0);
                    const totalRevenue = Number(row.rec_bilheteria_total || 0);
                    const doorRevenue = Number(row.rec_porta || 0);
                    const onlineRevenue = Number(row.rec_ticketeria || 0);
                    ...
                    return {
                        ...
                        name: row.evento,
                        date: row.data,
                        location: row.local || 'Não Definido',
                        ...
                        doorRevenue,
                        onlineRevenue,
                        barRevenue: Number(row.rec_bar_final || 0),

                        avgAge: Number(row.idade_media || 0),

                        // Calculated Fields to satisfy interface
                        percentPaying,
                        avgTicket,
                        cortesias: Math.max(0, totalAudience - totalPayingQty),
                        ...
                        dailySales: [],
                        preSaleQty: 0,
                        preSaleRevenue: onlineRevenue,

                        // Fields not returned by View but needed by UI
                        // We can default them or fetch separately if critical
                        investment: Number(row.custo_total || 0),
                        roi: Number(row.roi_percent || 0),
                        ...
                        ticketsSold: totalPayingQty,

                        // Coupons are not in view, maybe fetch separately if needed
                        coupons: {}
                    };
                });
        ...
        loadData();
    }, []); // Re-run if selected ID changes? No, only once. Removed dependency.

    const currentEvent = useMemo(() =>
        events.find(e => e.id === selectedEventId) || events[0]
        , [events, selectedEventId]);

    // Derived Data for Charts - Moved inside conditional or safely accessed
        ...
        <div className="space-y-8 animate-in fade-in duration-500 pb-10">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                ...
                <div className="flex flex-wrap items-center gap-3">
                    {/* Data Management Modal - ALWAYS VISIBLE */}
                    <DataManagement
            ...
            {/* Empty/Loading State */}
            {!currentEvent ? (
                <div className="flex flex-col items-center justify-center p-20 border-2 border-dashed border-zinc-200 rounded-3xl bg-zinc-50/50 dark:border-zinc-800 dark:bg-zinc-900/30">
                    <p className="text-lg font-medium text-zinc-500 mb-4 dark:text-zinc-400">Nenhum dado encontrado.</p>
                    <p className="text-sm text-zinc-400 mb-8 max-w-md text-center">Use o botão "Importar Dados" acima para carregar sua planilha Consolidada ou CSVs do sistema.</p>
                </div>
            ) : (
                <>
